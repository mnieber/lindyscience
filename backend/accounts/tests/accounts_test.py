...
class AccountTest(APITestCase):
    ...
    def test_create_and_activate_user(self):
        ...
        urls = set(
            [
                url
                for url in [a["href"] for a in html.find_all("a")]
        ...
        res = self.client.post("/auth/users/confirm/", {"uid": uid, "token": token,})
    ...
    def test_log_in_new_user(self):
        ...
        res = self.client.post(
            "/auth/token/login/", {"email": test_email, "password": "87654321"}
        )
        assert res.status_code == status.HTTP_400_BAD_REQUEST
        assert (
            res.data["non_field_errors"][0].lower()
            == "unable to log in with provided credentials."
        )

        # Log in the newly created user
        res = self.client.post(
            "/auth/token/login/", {"email": test_email, "password": test_password}
        )
        ...
        res = self.client.post(
            "/auth/users/create/",
            {"email": test_email, "password": test_password, "accepts_terms": "true"},
        )
    ...
    def test_create_user_account_no_tos(self):
        ...
        res = self.client.post(
            "/auth/users/create/",
            {"email": test_email, "password": test_password, "accepts_terms": "false"},
        )

        assert res.data["accepts_terms"][0].lower() == "required_to_accept_terms"
        assert res.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_register_with_empty_fields(self):
        ...
        res = self.client.post(
            "/auth/users/create/",
            {"email": "", "password": test_password, "accepts_terms": "true"},
        )
        ...
        res = self.client.post(
            "/auth/users/create/",
            {"email": test_email, "password": "", "accepts_terms": "true"},
        )
        assert res.data["password"][0].lower() == "this field may not be blank."
        assert res.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_password_reset(self):
        ...
        res = self.client.post("/auth/password/reset/", {"email": test_email,})
        ...
        urls = set(
            [
                url
                for url in re.findall(self.url_regex, html_content)
        ...
        res = self.client.post(
            ...
            {"uid": uid, "token": token, "new_password": "foobar",},
        )
        self.assertEqual(res.status_code, status.HTTP_204_NO_CONTENT)

        res = self.client.post(
            "/auth/token/login/", {"email": test_email, "password": "foobar"}
        )
        assert res.status_code == status.HTTP_200_OK
