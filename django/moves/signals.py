...
def create_profile_on_activation(sender, user, request, **kwargs):
    trash = models.MoveList(role='trash',
                            name='Trash',
                            slug='trash',
                            is_private=True,
                            description='',
    ...
    drafts = models.MoveList(role='drafts',
                             name='Drafts',
                             slug='drafts',
                             is_private=True,
                             description='',
    ...
    profile = Profile(owner=user,
                      recent_move_url='lists/%s/%s' %
                      (user.username, moves.name))
    profile.save()

    for (idx, move_list) in enumerate([trash, drafts, moves]):
        p2m = ProfileToMoveList(profile=profile,
                                move_list=move_list,
                                order=idx + 1)
        p2m.save()
