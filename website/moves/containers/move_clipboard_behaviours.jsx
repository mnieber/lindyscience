...
export function useMoveClipboard(
  ...
  actInsertMoves: (moveIds: Array<UUID>, moveList: UUID, targetMoveId: UUID) => Array<UUID>,
  ...
  function shareToList(targetMoveList: MoveListT) {
    if (isTarget(targetMoveList)) {
      const moveIdsInTargetMoveList = actInsertMoves(
        selectedMoveIds, targetMoveList.id, ""
      );
      api.saveMoveOrdering(targetMoveList.id, moveIdsInTargetMoveList)
  ...
  function moveToList(sourceMoveList: MoveListT, targetMoveList: MoveListT) {
    ...
    api.saveMoveOrdering(sourceMoveList.id, newMoveIds)
      .catch(createErrorHandler("Could not update the move list"));

    const newIdx =
      findNeighbourIdx(newMoveIds, selectedMoveIds, anchorIdx, selectedMoveIds.length, 1) ||
